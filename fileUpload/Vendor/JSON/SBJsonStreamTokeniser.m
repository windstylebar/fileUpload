...
- (sbjson_token_t)match:(char *)str retval:(sbjson_token_t)tok token:(char **)token length:(NSUInteger *)length {
...
- (sbjson_token_t)getStringToken:(char **)token length:(NSUInteger *)length {

    // Skip initial "
    index++;

    NSUInteger string_start = index;
    sbjson_token_t tok = sbjson_token_string;
...
- (sbjson_token_t)getNumberToken:(char **)token length:(NSUInteger *)length {
    ...
    sbjson_token_t tok = sbjson_token_integer;
    if (bytes[index] == '0') {
        index++;
...
- (sbjson_token_t)getToken:(char **)token length:(NSUInteger *)length {
    [self skipWhitespace];
    ...
    switch (ch) {
        case '{': {
            index++;
            tok = sbjson_token_object_open;
            break;
        }
        case '}': {
            index++;
            tok = sbjson_token_object_close;
            break;

        }
        case '[': {
            index++;
            tok = sbjson_token_array_open;
            break;

        }
        case ']': {
            index++;
            tok = sbjson_token_array_close;
            break;

        }
        case 't': {
            tok = [self match:"true" retval:sbjson_token_bool token:token length:length];
            break;

        }
        case 'f': {
            tok = [self match:"false" retval:sbjson_token_bool token:token length:length];
            break;

        }
        case 'n': {
            tok = [self match:"null" retval:sbjson_token_null token:token length:length];
            break;

        }
        case ',': {
            index++;
            tok = sbjson_token_value_sep;
            break;

        }
        case ':': {
            index++;
            tok = sbjson_token_entry_sep;
            break;
