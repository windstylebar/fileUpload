...
- (NSString *)requestURI
{
    ...
    NSString *tempString = [[request url] relativeString];
    
    NSString *returnVale = [[tempString  stringByReplacingOccurrencesOfString:@"+" withString:@" "]
                            stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    HTTPLogVerbose(@"requestURI|%@",returnVale);
	return returnVale;
}
...
- (NSString *)dateAsString:(NSDate *)date
{
    ...
    static NSString *localDataString = nil;
    ...
    if ([localDataString length] ==0) {
        localDataString = [df stringFromDate:date];
    }
...
- (id)initWithServer:(HTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot uploadRoot:(NSString *)upRoot queue:(dispatch_queue_t)q
{
	if ((self = [super init]))
	{
		server = aServer;
        ...
        if(upRoot)
        {
            uploadRoot = [upRoot stringByStandardizingPath];
        }
