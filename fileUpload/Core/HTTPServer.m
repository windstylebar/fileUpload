...
- (void)setUpLoadRoot:(NSString *)value
{
    HTTPLogTrace();
...
- (BOOL)start:(NSError **)errPtr
{
	...
	dispatch_sync(serverQueue, ^{ @autoreleasepool {
		...
		if (success)
		{
            HTTPLogInfo(@"%@: Started HTTP server on IP addr %@", THIS_FILE, [asyncSocket localHost]);
...
//	return [[HTTPConfig alloc] initWithServer:self documentRoot:documentRoot queue:connectionQueue];
    ...
    return [[HTTPConfig alloc] initWithServer:self documentRoot:documentRoot uploadRoot:upLoadRoot queue:connectionQueue];
